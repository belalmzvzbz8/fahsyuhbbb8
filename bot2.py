...
stopuser = {}
token = '6538835610:AAHJ7u8OaQXjieiq1MaP1ogDdgzlOe66yz4'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/the_FJU")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/the_FJU")
		keyboard.add(contact_button)
...
𝘽𝙤𝙩 𝘽𝙮 @the_FJU''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='MO3GZA-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
